...
function message(api, event) {
        ...
        function handleSendError(err, threadID) {
                const errString = JSON.stringify(err);
                if (errString.includes('spam')) {
                        setErrorUptime();
                        throw err;
                }
                if (errString.includes('1545012')) {
                        console.log(`[message] Error 1545012 for thread ${threadID} - Cannot send to this conversation (permission/rate limit issue). Message suppressed.`);
                        return null;
                }
                throw err;
        }
        ...
        return {
                send: async (form, callback) => {
                        try {
                                ...
                                return handleSendError(err, event.threadID);
                        }
                },
